...
`bootstrap.sh` at the repository root (deployment-excluded) is the fast path for a brand-new Linux machine: it installs chezmoi and the 1Password CLI when missing, verifies GitHub's Ed25519 host key against `api.github.com/meta`, clones or fast-forward-updates `~/.chezmoi` over SSH, creates or merges the local chezmoi config, and then applies the dotfiles with `--force --keep-going`. Existing managed targets and their symlink definitions are backed up under `~/.chezmoi-backups/` before anything is overwritten. If no `~/.zshrc` exists it creates one from `examples/zshrc` (still unmanaged); an existing one is left for manual merging. Pass `--op-service` to add the `[onepassword]` service block, or `--no-apply` to stop before applying. It is idempotent and aborts on unexpected existing state (wrong remote, dirty tree). It never installs mise, antidote, vim-plug, fonts, or other tooling; the steps below still cover everything it does not do. Do not run it casually on an already-configured peer machine.
