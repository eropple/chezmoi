...
- `bootstrap-linux.sh` and `bootstrap-mac.sh` are the only sanctioned installers: they install chezmoi, the 1Password CLI, and mise, and run `mise install`. The Linux script downloads chezmoi and mise release artifacts with pinned SHA-256 checksums; the macOS script installs the same tools through an already-installed Homebrew, which performs its own verification. No other automatic install hooks or external downloads are configured; antidote, vim-plug, and fonts remain separate explicit steps.

## Layout

`dot_config/zsh-ng/` deploys to `~/.config/zsh-ng/`; `dot_vimrc` to `~/.vimrc`; `dot_gitconfig` to `~/.gitconfig`; `dot_bazelrc` to `~/.bazelrc`, capping local Bazel build actions at half the host's logical CPUs (`--local_resources=cpu=HOST_CPUS*.5`, current Bazel 9 syntax; workspaces may override); and so on. `dot_config` also contains mise, Ghostty, and Zellij settings. `dot_tmux.conf` and `dot_editorconfig` are shared.

`AGENTS.md`, `README.md`, `bootstrap-linux.sh`, `bootstrap-mac.sh`, `examples/`, and `knowledge-base/` are excluded from deployment by `.chezmoiignore`. `examples/zshrc` is an example to merge manually, never a managed target. `knowledge-base/` contains shared operational guides for consuming agents, not automatic provisioning: distinguish host services and cluster infrastructure from optional application workloads, confirm machine-specific prerequisites with the operator, and obtain approval before changing services or deploying resources.
...
`bootstrap-linux.sh` at the repository root (deployment-excluded) is the fast path for a brand-new Linux machine: it installs chezmoi, the 1Password CLI, and mise (checksum-verified) when missing, verifies GitHub's Ed25519 host key against `api.github.com/meta`, clones or fast-forward-updates `~/.chezmoi` over SSH, creates or merges the local chezmoi config, applies the dotfiles with `--force --keep-going`, and runs `mise install` for the declared shell tools. `bootstrap-mac.sh` is the macOS counterpart with the same flow, flags, and backups; it installs chezmoi, the 1Password CLI, and mise through Homebrew instead of pinned downloads (Homebrew must already be installed), and additionally backs up and removes directory symlinks lying on a managed target's path (such as legacy `~/.config/ghostty` or `~/.config/zellij` links into an old dotfiles checkout) so apply writes real files instead of through the link into the legacy tree. Existing managed targets and their symlink definitions are backed up under `~/.chezmoi-backups/` before anything is overwritten. If no `~/.zshrc` exists a script creates one from `examples/zshrc` (still unmanaged); an existing one is left for manual merging. Pass `--op-service` to add the `[onepassword]` service block (added automatically when `OP_SERVICE_ACCOUNT_TOKEN` is set), or `--no-apply` to stop before applying. Both are idempotent, exit non-zero with a loud summary when any step fails, and abort on unexpected existing state (wrong remote, dirty tree). They never install antidote, vim-plug, fonts, or other tooling; the steps below still cover everything they do not do. Do not run them casually on an already-configured peer machine.
...
6. Tools come from mise, installed and activated by the bootstrap scripts, which also run `mise install` against the deployed `~/.config/mise/config.toml`. On machines set up before that change or by hand, install mise using its current official instructions, then run `mise install`. Declared versions use `latest`; they are not a reproducible lock. Without mise (or before `mise install` finishes), the shared shell intentionally degrades to a plain prompt: starship, zoxide, fzf, direnv, and zellij will be missing.
...
10. Provision secrets separately. Machines using the shared 1Password service account keep its token at `~/.local/1password_token` (mode 0600, never committed, never copied between machines via Git). The shared zsh tools init exports it as `OP_SERVICE_ACCOUNT_TOKEN` when present and not already set, and the bootstrap scripts enable `[onepassword]` service mode automatically when that variable is set in their environment. Never commit the token file or resolved secret values, and decide deliberately which machines get the token at all.
...
Polytoken rewrites the deployed `config.yaml` by itself whenever settings change through the TUI or daemon (model picks, permission prompts, daemon tuning), leaving timestamped `config.yaml.<UTC>.bak` siblings and re-serializing with its own field order (telemetry serializes as `protocol`, `endpoint`, `headers`, `service_name`, with the Honeycomb header unquoted). Treat that target-side churn as the normal drift signal, not a hand edit: fold the semantic change into `.chezmoitemplates/polytoken-config.yaml` by hand, keeping the `onepasswordRead` telemetry reference intact, and never import the rendered file. A `chezmoi diff` against a Polytoken-rewritten target can therefore show cosmetic ordering/quoting noise with no semantic change; verify by comparing parsed YAML, not bytes. When the `op` CLI is unavailable in the current shell (it is absent from PATH non-interactively on at least one machine), verify with the secret stubbed in a scratch copy of the source: copy the checkout, replace the `onepasswordRead` expression with a literal, then `chezmoi --source <copy> cat ~/.config/polytoken/config.yaml` and compare against the live file with the header masked. As of 2026-09 the shared config tracks schema `version: 4`: top-level `modelgroups` with namespaced keys `polytoken:default_model_full` and `polytoken:default_model_mini` replaces the former `version: 3` `defaults: full/mini` pair; providers are codex (device auth), deepinfra, google-ai, zai, and xiaomi (`${MIMO_API_KEY}`, which replaced `ollama-cloud`/`${OLLAMA_API_KEY}`); search enables brave, tavily, exa, parallel, and you alongside disabled kagi.
...
Shared reusable SOPS-encrypted assets belong in `secrets/`, named `*.sops.yaml` where appropriate. This entire directory is chezmoi-ignored: consume ciphertext from `~/.chezmoi/secrets/`, never automatically deploy or decrypt it. See `secrets/README.md` for the standard workflow. Obtain `SOPS_AGE_KEY` from `op://Local Dev/ed3dnet ed3d.net localdev age key/password` using an authorized 1Password CLI session or separately provisioned service-account token; never print, commit, or store the private key in chezmoi. `secrets/cloudflare-api-token.sops.yaml` is the canonical shared Cloudflare Secret; cluster-specific passwords remain owned by their infrastructure repo. This checkout has no SOPS creation rules: new assets need an explicit reviewed recipient/encryption policy. Moving ciphertext does not require decryption. Approval is still required before applying decrypted values to any destination.

`private_dot_env.op` deploys `~/.env.op` with mode 0600. It contains only 1Password references, never resolved values; as of 2026-09 it carries the provider/search keys (`LITELLM_API_KEY`, `MIMO_API_KEY`, `PARALLEL_API_KEY="op://Local Dev/Parallel/apikey"`, `YOU_API_KEY="op://Local Dev/You/apikey"`, and similar) and no longer carries `HONEYCOMB_API_KEY`. Keep reference identifiers private when sharing the repository. The shared zsh tools init loads it in interactive shells: it exports `OP_SERVICE_ACCOUNT_TOKEN` from `~/.local/1password_token` when present and not already set, then resolves the file with `op inject` and exports the results. Loading skips silently when op is missing/unauthorized or the file is absent, and `ZSH_NG_NO_OP_ENV=1` opts out. Because a machine's service account may not be able to read every referenced item, individual keys can silently resolve to nothing; verify with `op whoami` and per-item reads when a key matters. The Polytoken telemetry template resolves the Honeycomb reference directly at chezmoi render time and does not depend on `.env.op` being loaded.
