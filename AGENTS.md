...
- `bootstrap.sh` is the only sanctioned installer: it installs chezmoi, the 1Password CLI, and mise (checksum-verified) and runs `mise install`. No other automatic install hooks or external downloads are configured; antidote, vim-plug, and fonts remain separate explicit steps.
...
`bootstrap.sh` at the repository root (deployment-excluded) is the fast path for a brand-new Linux machine: it installs chezmoi, the 1Password CLI, and mise (checksum-verified) when missing, verifies GitHub's Ed25519 host key against `api.github.com/meta`, clones or fast-forward-updates `~/.chezmoi` over SSH, creates or merges the local chezmoi config, applies the dotfiles with `--force --keep-going`, and runs `mise install` for the declared shell tools. Existing managed targets and their symlink definitions are backed up under `~/.chezmoi-backups/` before anything is overwritten. If no `~/.zshrc` exists it creates one from `examples/zshrc` (still unmanaged); an existing one is left for manual merging. Pass `--op-service` to add the `[onepassword]` service block (added automatically when `OP_SERVICE_ACCOUNT_TOKEN` is set), or `--no-apply` to stop before applying. It is idempotent, exits non-zero with a loud summary when any step fails, and aborts on unexpected existing state (wrong remote, dirty tree). It never installs antidote, vim-plug, fonts, or other tooling; the steps below still cover everything it does not do. Do not run it casually on an already-configured peer machine.
...
6. Tools come from mise, installed and activated by `bootstrap.sh`, which also runs `mise install` against the deployed `~/.config/mise/config.toml`. On machines set up before that change or by hand, install mise using its current official instructions, then run `mise install`. Declared versions use `latest`; they are not a reproducible lock. Without mise (or before `mise install` finishes), the shared shell intentionally degrades to a plain prompt: starship, zoxide, fzf, direnv, and zellij will be missing.
...
10. Provision secrets separately. Machines using the shared 1Password service account keep its token at `~/.local/1password_token` (mode 0600, never committed, never copied between machines via Git). The shared zsh tools init exports it as `OP_SERVICE_ACCOUNT_TOKEN` when present and not already set, and `bootstrap.sh` enables `[onepassword]` service mode automatically when that variable is set in its environment. Never commit the token file or resolved secret values, and decide deliberately which machines get the token at all.
